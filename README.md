...
- 미션 가이드에서 PostgreSQL과 psql 환경 사용을 권장하여, 해당 환경을 기준으로 작성했습니다. 따라서 MySQL 전용 문법은 사용하지 않았습니다.
- `BETWEEN`은 경우에 따라 정확도가 떨어질 수 있다고 판단하여, 초급 8번과 고급 2번 문제에서 정확성 확보를 위해 범위 조건을 명시했습니다.
  - 예시) `WHERE date BETWEEN '2025-03-01' AND '2025-03-31'` 대신 `WHERE date >= '2025-03-01' AND date < '2025-04-01'`
  - 단, 성능 측면에서는 `BETWEEN`보다 떨어질 수 있다는 점도 고려하고 있습니다.
- 정합성과 성능을 고려하여 `SELECT *` 대신 명시적 컬럼 지정을 사용했습니다.
  - 데이터가 API와 연동될 가능성이 있어, 데이터 정합성 및 컬럼 순서 유지가 중요하다고 판단했습니다.
  - 불필요한 컬럼 조회를 방지하고, 유지보수성을 높이기에 유리하다고 판단했습니다.
- 위 2건에 관한 멘토님 의견이 궁금합니다.