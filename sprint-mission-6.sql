...
SELECT *
FROM orders
WHERE 2025 = EXTRACT('YEAR' from date) AND 3 = EXTRACT('MONTH' from date)
;
...
SELECT pizza_id, COUNT(order_id) AS "주문 건수"
FROM order_details
GROUP BY pizza_id;
;
...
SELECT pizza_id, SUM(quantity) AS "주문 수량"
FROM order_details
GROUP BY pizza_id;
;
...
SELECT pizza_id, SUM(quantity * p.price) AS "total_revenue"
...
/*
    1. 피자별(`pizzas.id` 기준) 판매 수량 순위에서 피자별 판매 수량 상위에 드는 베스트 피자를 10개를 조회해 주세요. `pizzas`의 모든 컬럼을 조회하면서 각 피자에 해당하는 판매량을 `total_quantity`라는 이름으로 함께 조회합니다.
        ...
        ```
*/
SELECT pizzas.*, tq.total_quantity 
FROM pizzas
JOIN (
	SELECT pizza_id, SUM(quantity) AS "total_quantity"
	FROM order_details
	GROUP BY pizza_id 
) tq ON (id = tq.pizza_id)
...
SELECT 
	o.date,
	COUNT(DISTINCT o.id) AS total_amount,
	SUM(quantity * p.price) AS "order_revenue"
FROM orders o
JOIN order_details od ON (o.id = od.order_id)
...
SELECT 
	o.date, 
	COUNT(pd.order_id) AS total_orders,
	SUM(order_revenue) AS total_amount
FROM orders o
JOIN (
	SELECT 	order_id, 
...
SELECT 
	p.size,
	SUM(od.quantity * price) AS "사이즈 별 수익"
FROM order_details od
JOIN pizzas p ON(od.pizza_id = p.id)
GROUP BY p.size
ORDER BY "사이즈 별 수익" DESC
;
/*    
...
SELECT
	pt.name,
	SUM(od.quantity * price) AS "피자 종류별 수익"
FROM order_details od
JOIN pizzas p ON(od.pizza_id = p.id)
JOIN pizza_types pt ON (p.type_id = pt.id)
GROUP BY pt.name
ORDER BY "피자 종류별 수익" DESC
;
