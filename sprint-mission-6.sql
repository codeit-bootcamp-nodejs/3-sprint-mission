...
SELECT *
FROM orders
WHERE EXTRACT(YEAR FROM date) = 2025 AND EXTRACT(MONTH FROM date) = 3
;
-- 9. `orders` 테이블에서 2025년 3월 12일 오전에 주문된 내역만 조회하세요.
SELECT *
FROM orders
WHERE EXTRACT(YEAR FROM date) = 2025 AND EXTRACT(MONTH FROM date) = 3 AND EXTRACT(DAY FROM date) = 12
	AND (EXTRACT(HOUR FROM time) < 12)
;
-- 10. `pizza_types` 테이블에서 이름에 'Cheese' 혹은 'Chicken'이 포함된 피자 종류를 조회하세요. (대소문자를 구분합니다)
SELECT *
FROM pizza_types
WHERE POSITION('Cheese' IN pizza_types.name) > 0 OR POSITION('Chicken' IN pizza_types.name) > 0
;
-- # 중급 문제

-- 1. `order_details` 테이블에서 각 피자(`pizza_id`)별로 주문된 건 수(`order_id`)를 보여주세요.
SELECT pizza_id, 
	COUNT(id) AS "order_id"
FROM order_details
GROUP BY pizza_id
;
...
SELECT *
FROM order_details
WHERE order_details.pizza_id IN (
	SELECT id
	FROM pizzas
	WHERE price > 20
)
...
SELECT o.pizza_id, 
	SUM(o.quantity * p.price) AS total_revenu 
...
FROM pizzas AS p
JOIN order_details AS o ON p.id=o.pizza_id
GROUP BY id, type_id, size
...
JOIN pizzas AS p ON p.id = d.pizza_id
GROUP BY date
HAVING EXTRACT(YEAR FROM o.date) = 2025 AND EXTRACT(MONTH FROM o.date) = 3
;
/*
...
SELECT pt.name AS pizza_name,
    ...
    (p.price * d.quantity) AS total_amount
FROM order_details AS d
JOIN pizzas AS p ON d.pizza_id = p.id
