...
WHERE 
	date_part('YEAR', date) = 2025
	AND date_part('MONTH', date) = 3
	AND date_part('DAY', date) = 12
	AND date_part('HOUR', date) <= 12
;
...
FROM order_details
JOIN pizzas ON (pizza_id = pizzas.id)
GROUP BY pizza_id
;
...
SUM(od.quantity * pizzas.price) AS "total_amount"
FROM orders AS o
INNER JOIN order_details AS od ON o.id = od.order_id
INNER JOIN pizzas ON (od.pizza_id = pizzas.id)
WHERE DATE_PART('YEAR', o.date) = 2025
AND DATE_PART('MONTH', o.date) = 3
GROUP BY date
ORDER BY date ASC
;
